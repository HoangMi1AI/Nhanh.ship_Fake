    ...
    internal class Class_Diachi
    {
        ...
        public void LoadDistricts(ComboBox cboDistricts, string provinceCode)
        {
            using (MySqlConnection connection = GetMySqlConnection())
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT code, name FROM districts WHERE province_code = @provinceCode";
                    MySqlDataAdapter adapter = new MySqlDataAdapter(query, connection);
        ...
        public void LoadXa(ComboBox cboXa, string district_code)
        {
            using (MySqlConnection connection = GetMySqlConnection())
            {
                try
                {
                    connection.Open();
                    string query = "SELECT name FROM wards WHERE district_code = @district_code";
                    MySqlDataAdapter adapter = new MySqlDataAdapter(query, connection);
                    adapter.SelectCommand.Parameters.AddWithValue("@district_code", district_code);
                    DataTable dt = new DataTable();
                    adapter.Fill(dt);

                    // Thêm mục mặc định vào đầu danh sách
                    DataRow row = dt.NewRow();
                    row["name"] = "Chọn xa";  // Tên mục mặc định
                    dt.Rows.InsertAt(row, 0);
                ...
                {
                    MessageBox.Show("Error loading xa: " + ex.Message);
                }
            }
        }
    }
